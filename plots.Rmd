...
ggplot(mem_data,aes(x=block,y=correct))+
  ...
  theme(legend.position="none",
        ...
        axis.text = element_text(size = 14))+
  xlab("Learning Block")+
  ylab("Proportion of Subsequent Memory")
...
p+geom_line(aes(color=as.factor(subject)),alpha=.7)+
  stat_sum_single(mean,geom="line")+
  theme_classic()+
  theme(legend.position="none",
        axis.title = element_text(size = 18),
...
p2+geom_line(aes(color=as.factor(subject)),alpha=.7)+
  stat_sum_single(mean,geom="line")+
  theme_classic()+
  theme(legend.position="none",
        axis.title = element_text(size = 18),
...
gplot<-a$plot.mp[[1]]+theme_bw()+theme(legend.position="none")+
...
theme(axis.title=element_text(size=20,face="bold"))+theme(axis.text=element_text(size=18))
...
gplot2<-a2$plot.mp[[1]]+theme_bw()+theme(legend.position="none")+
  #ggtitle("Effect of Flexibility on Learning")+
  ylab("Proportion of Correct Response")+
...
rl_flex<-as.data.frame(cbind(tapply(flex_behav$wb_flex,flex_behav$subject,mean),
                             tapply(flex_behav$str_flex,flex_behav$subject,mean),rl_data$alpha))
names(rl_flex)<-c("wb_flex","str_flex","alpha")
rl_flex$alpha_rank<-rank(rl_data$alpha)
