function [S,Q,B] = multicat_res_signed_norm(A,gplus,gminus,omega)
...
for s=1:T
    ...
    kminus=sum(Aminus)';
    indx=[1:N]+(s-1)*N;    
    B(indx,indx)=A{s}-(gplus*(kplus*kplus')/sum(kplus)-gminus*(kminus*kminus')/sum(kminus));
    Asum(s) = sum(sum(A{s}));
end
...
norm = sum(Asum) + numel(find(spdiags(ones(N*T,2*T-2),all2all,N*T,N*T)));
S = reshape(S,N,T);
Q = full(Q)./norm;
