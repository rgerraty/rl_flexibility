...
if [ -z $2 ]
	...
	fslmeants -i $feat/filtered_func_data.nii.gz -o $feat/gm.txt -m $feat/reg/gm;
	...
	if [ -e $feat/spike_regressors_$sd_thresh\_SD.txt ];
		then 
		paste -d "  " $feat/36_par_conf.txt $feat/spike_regressors_$sd_thresh\_SD.txt>$feat/36par+spikes.txt; 
		paste -d "  " $feat/18_par_conf.txt $feat/spike_regressors_$sd_thresh\_SD.txt >$feat/18par+spikes.txt;
		paste -d "  " $feat/24_par_conf.txt $feat/spike_regressors_$sd_thresh\_SD.txt >$feat/24par+spikes.txt;
		paste -d "  " $feat/12_par_conf.txt $feat/spike_regressors_$sd_thresh\_SD.txt>$feat/12par+spikes.txt;
	else 
